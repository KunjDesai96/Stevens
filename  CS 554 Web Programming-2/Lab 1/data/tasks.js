...
const getTaskByID = async function getTaskByID(id) {
...
const getAllTasks = async function getAllTasks() {
  ...
  //     result.push(await getTaskByID(element._id.toString()));
  // }
  return allTasks;
};
...
const updateTaskByPut = async function updateTaskByPut(
  id,
  title,
  description,
  hoursEstimated,
  ...
  const tasksCollection = await tasks();
  var o_id = new ObjectId(id);
  let recentTask = await getTaskByID(id);
  ...
  let recentNewTask = await getTaskByID(id);
...
const deleteComment = async function deleteComment(taskid, commentid) {
  ...
  const deletedComment = await getCommentByID(commentid);

  const deleteInfo = await commentsCollection.findOneAndDelete({ _id: c_o_id });
